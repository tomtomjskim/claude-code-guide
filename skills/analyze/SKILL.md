...
낯선 코드베이스나 unfamiliar module을 분석할 때는 바로 수정 포인트를 단정하지 말고
먼저 `zoom-out`을 수행합니다.

1. 관련 entry point와 public interface를 찾습니다.
2. 호출자와 의존 모듈을 1-2단계만 추적합니다.
3. 변경 후보 파일과 건드리지 않을 파일을 분리합니다.
4. 기존 테스트 또는 검증 명령을 확인합니다.
5. 구현 전에 리스크와 검증 방법을 실행 전략 추천에 포함합니다.

출력은 긴 탐색 로그가 아니라 변경 판단에 필요한 코드 지도여야 합니다.
...
- Entry point: {요청이 들어오는 시작점}
- Public interface: {API/UI/service contract}
- 건드리지 않을 영역: {범위 제외 파일/모듈}
- 검증 루프: {테스트/명령/수동 확인}
