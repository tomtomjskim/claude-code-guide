...
### Phase 1A: Diagnose Loop (버그/회귀 대상)

원인 불명 버그, flaky failure, 회귀 의심 변경이면 Phase 2로 바로 넘어가지 말고
다음을 먼저 기록합니다.

1. 증상을 한 문장으로 정의합니다.
2. 가장 빠른 재현 루프를 고릅니다. 테스트, script, curl, UI 조작 중 결정적인 방법을 우선합니다.
3. 원인 가설을 3-5개로 제한합니다.
4. 각 가설의 관찰 신호와 기각 조건을 둡니다.
5. 검증은 한 번에 하나의 변수만 바꿉니다.

재현 루프가 없으면 `PASS`로 종결하지 않습니다. 이 경우 결과는
`NEEDS-INFO` 또는 `BLOCKED`로 남기고 필요한 입력을 명시합니다.
