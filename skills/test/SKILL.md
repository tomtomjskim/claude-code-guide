...
5. **Tracer Bullet**: 큰 기능은 작은 vertical slice별로 실패 확인 → 최소 구현 → 리팩토링을 반복
...
- 첫 번째 tracer bullet slice 정의
  - public interface 기준으로 테스트할 진입점 선택
  - 실패가 사용자 가치와 직접 연결되는지 확인
  - 한 번에 대량의 실패 테스트를 만들지 않음
...
- **Public interface 우선**: 내부 구현보다 API, UI action, service contract 등 외부 관찰 가능한 동작 테스트
- **실패 확인 기록**: Red 상태에서 실제 실패한 테스트 이름과 이유를 남김
...
- **하드코딩된 데이터** (변수화)
- **horizontal batch 금지**: 전체 레이어 테스트를 한꺼번에 대량 생성하지 않음
- **green-only 테스트 금지**: 실패 확인 없이 통과하는 테스트만 추가하지 않음
...
4. **Tracer Bullet 요약**: 첫 slice, 실패 확인 방법, 다음 slice 후보
5. **다음 단계 제안**: `/run` 커맨드로 TDD 사이클 시작
...
- **한 slice씩 진행**: Red 테스트를 닫기 전 다음 테스트 묶음을 과도하게 만들지 않음
